...
const PendingRequestsTable = ({ onRequestProcessed }) => {
  ...
  const handleApprove = async (requestId) => {
    ...
    try {
      ...
      if (result) {
        showNotification(
          "Request approved and user created successfully",
          "success"
        );
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {pendingRequests.map((request) => (
              ...
              >
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <select
                    className={`text-sm border border-gray-300 rounded px-2 py-1 ${getRoleColorClass(
                      selectedRoles[request._id] || "VIEWER"
                    )}`}
                    onChange={(e) =>
